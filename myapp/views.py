...
from .forms import UserLoginForm, UserRegisterForm,BusInfo
...
def CreateBusList(request):
    form=BusInfo()
    if request.user.is_authenticated:
        if request.method== "POST":
            form=BusInfo(request.POST)
            if form.is_valid():
                form.save()
    else:
        return redirect('signin')            
    context={'form':form}
    return render(request,'myapp/updatecreatebus.html',context)
